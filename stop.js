...
.then((client) => {
    ...
    console.log(new Date().toString() + ': Starting STOP on '+CSAEQUIP)
...
function CheckAction(client,Id){
    console.log(new Date().toString() + ': Check State of STOP on '+CSAEQUIP)
    ...
    .then((result) => {
       ...
       setTimeout(() => {
        ...
        {    
        console.log(new Date().toString() + ': STOP done on '+CSAEQUIP)
        }else{
            throw (result[0])
            process.exit(5);
        }
...
+
    ...
    <key>action</key>\
    <value xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" \
        xmlns:xs="http://www.w3.org/2001/XMLSchema" xsi:type="xs:string">stop</value>\
    </entry>'
+
    '</args></ns1:execAction>';
